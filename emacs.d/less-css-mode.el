...
(defcustom less-compile-jump-to-error t
...
(defun less-command-compile (file-name)
  (mapconcat 'identity
              (append (list (shell-quote-argument less-css-lessc-command))
                      less-css-lessc-options
...
(defun less-css-compile ()
  ...
  (interactive)
  (let ((compiler-output (shell-command-to-string (less-command-compile (buffer-file-name)))))
    (if (string= compiler-output "")
        (message "Compiled and saved %s" (less-css--output-path))
      (let* ((msg (car (split-string compiler-output "[\n\r]+")))
	     (line (and (string-match "^ParseError.+:\\([0-9]+\\):\\([0-9]+\\)$" msg)
			(string-to-number (match-string 1 msg)))))
	(message msg)
	(when (and less-compile-jump-to-error line (> line 0))
	  (goto-char (point-min))
	  (forward-line (1- line)))))))
